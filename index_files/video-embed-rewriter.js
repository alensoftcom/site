...
var _wmVideos_ = new function () {
  ...
  class VideoPlayer {
    ...
    discoverVideoInfo() {
      ...
      if (this.vinfo && !this.vinfo.location && !this.vinfoFetch) {
        this.vinfoFetch = this.getCDXVideoInfo().then(() => {
          if (!this.replaced) {
            this.replacePlayer();
          }
        })
    ...
    async getCDXVideoInfo() {
      ...
      const vinfo = this.vinfo;
      const url = '/__wb/videoinfo?vtype=youtube&vid=' + vinfo.vid;
      ...
      if (response.status == 200) {
        ...
        if (meta && meta.status_code != null) {
          ...
          const status_text = VIDEO_STATUS_TEXT[meta.status_code];
          ...
          for (const video of meta.formats) {
      ...
      // Fallback to ES if vstream is not found by /__wb/videoinfo.
      if (vinfo.location == null) {
        await this.getVideoStreamInfo();
      }
    }
    async getVideoStreamInfo() {
      // Lookup using the ES index
      const vinfo = this.vinfo;
      const vstreamUrl = resourcekey('yt/' + vinfo.vid);
      const response = await fetch(vstreamUrl, {method: 'HEAD'});
      if (response.status != 200) {
        console.log('%s -> %s', vstreamUrl, response.status);
      ...
      var location = response.headers.get('content-location');
      vinfo.location = location || vstreamUrl;
    ...
    displayError(msg) {
      ...
      if (!e) {
        ...
        e.setAttribute('style', 'position:absolute;top:0;background:#000000c0;' +
          ...
          // this has unwanted side-effect on some versions of watch page.
